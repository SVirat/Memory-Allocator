...
void* Mem_Alloc(int size)
{
	...
	int pagesize = 4;
	int padsize = ((pagesize - (size % pagesize)) % pagesize) + size;
	int blockSize = padsize + (int)sizeof(block_header);
	block_header * head = list_head;
	int headSize = head->size_status;
	...
	while(head != NULL) {
		...
		if(head->next != NULL) {
			head = head->next;
			headSize = head->size_status;
		}
	}

	if(headSize - blockSize >= sizeof(block_header) + pagesize) {
		blockSize += sizeof(block_header);
		block_header * head3 = (block_header*) ((char*) head + blockSize);
		head3->size_status = blockSize - padSize - 2 * sizeof(block_header);
		headSize = padSize + 1;
...
int Mem_Free(void *ptr)
{
	...
	block_header * head = list_head;
	//the size of the bloc header
	int bsize = sizeof(block_header);
	//the size of the head
	int size = head->size_status;

	//return error at illegal input
	if(ptr == NULL) {
  		return -1;	
	}

	//getting to the block which should be freed
	while(head != NULL && (char*)head + bsize != ptr) {
  		head = head->next;
 	}

	//return error if head is null
	if(head == NULL) {
  		return -1;
	}
	...
	//the next block
	block_header* next = head->next;
	//the previous block, currently the head
	block_header* prev = list_head;

	//changing allocation status because block is freed
	if(size % 4 == 1) {
 		size--;
	}
	...
	//checking if first block is being freed
	if(prev != head){
		 ...
		 if(prev->size_status % 4 ==0) {
 			prev->next = head->next;
  		  	prev->size_status += head->size_status + bsize;
 		 } 
	}

	//getting to the next block if not null
	if(next != NULL) {
 		if(next -> size_status % 4 == 0) {
     			head->next = next->next;
     			size += next->size_status + bsize;
   		}
 	}

	//returning 0 if no error
	return 0;
